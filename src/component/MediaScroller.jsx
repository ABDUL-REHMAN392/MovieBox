...
// SkeletonCard now accepts darkTheme
function SkeletonCard({ darkTheme }) {
  return (
    <div className="w-40 flex-shrink-0">
      <div
        className={`w-full h-60 ${darkTheme ? "bg-gray-700" : "bg-gray-300"} animate-pulse rounded-xl mb-2`}
      ></div>
      <div
        className={`h-4 ${darkTheme ? "bg-gray-700" : "bg-gray-300"} animate-pulse rounded mb-1 w-3/4`}
      ></div>
      <div
        className={`h-4 ${darkTheme ? "bg-gray-700" : "bg-gray-300"} animate-pulse rounded w-1/2`}
      ></div>
    </div>
  );
}
 ...
 const { darkTheme } = useSelector(state => state.conditions);
  ...
  return (
    <div className="relative">
      ...
      <div
        ref={scrollRef}
        className="flex gap-6 overflow-hidden py-3 px-1"
